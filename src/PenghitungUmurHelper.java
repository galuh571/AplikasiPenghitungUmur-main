...
public void getPeristiwaBarisPerBaris(LocalDate tanggal, JTextArea jTextArea1, Supplier<Boolean> shouldStop) {
    try {
        ...
        for (int i = 0; i < events.length(); i++) {
            ...
            javax.swing.SwingUtilities.invokeLater(() ->
                jTextArea1.append(peristiwa + "\n")
            );
        }

        if (events.length() == 0) {
            javax.swing.SwingUtilities.invokeLater(() ->
                jTextArea1.setText("Tidak ada peristiwa penting yang ditemukan pada tanggal ini.")
            );
        }

    } catch (Exception e) {
        javax.swing.SwingUtilities.invokeLater(() ->
            jTextArea1.setText("Gagal mendapatkan data peristiwa: " + e.getMessage())
        );
    }
}
